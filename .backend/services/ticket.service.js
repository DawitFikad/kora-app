...
class TicketService {
    ...
    static async completePurchase(purchaseId) {
        ...
        const existingTicketsCount = await prisma_1.prisma.ticket.count({ where: { purchaseId } });
        ...
        for (let i = 0; i < quantity; i++) {
            ...
            const ticketId = crypto_1.default.randomUUID();
            ...
            const qrPayload = jsonwebtoken_1.default.sign(payload, secret);
            ticketsData.push({
                id: ticketId,
                code: ticketCode,
                qrPayload,
        ...
        // 1. Create tickets
        await prisma_1.prisma.ticket.createMany({
            data: ticketsData
        });
        // 2. Cleanup Redis Locks
        if (seatNumbers && seatNumbers.length > 0) {
            for (const seat of seatNumbers) {
                await lock_service_1.LockService.unlockSeat(eventId, tierId, seat);
            }
        }
        else {
            await lock_service_1.LockService.releaseCapacity(eventId, tierId, purchase.userId);
        }
        ...
        try {
            ...
            (async () => {
                try {
                    ...
                    const ticketsSold = await prisma_1.prisma.ticket.count({
     ...
     */
    static async invalidateTicket(ticketId, reason) {
        return prisma_1.prisma.ticket.update({
            where: { id: ticketId },
            data: {
                status: "CANCELLED",
