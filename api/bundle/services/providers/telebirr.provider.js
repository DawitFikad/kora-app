...
class TelebirrProvider {
    ...
    static getGatewayUrl() {
        return env_1.env.teleBirrApiUrl || 'https://196.188.120.3:38443/apiaccess/payment/gateway';
    }
    /**
    ...
    static formatPrivateKey() {
        try {
            const key = env_1.env.teleBirrPrivateKey;
            if (!key) {
            ...
            logger_1.default.error({ error: error.message }, 'Failed to format private key');
    ...
    static generateSignature(params) {
        try {
            ...
            logger_1.default.error({ error: error.message, params }, 'Signature generation failed');
    ...
    static async getToken() {
        try {
            logger_1.default.info('Requesting Telebirr fabric token');
            ...
            const response = await axios_1.default.post(`${gatewayUrl}/payment/v1/token`, {
                appSecret: env_1.env.teleBirrAppSecret
            }, {
                headers: {
                    'Content-Type': 'application/json',
                    'X-APP-Key': env_1.env.teleBirrFabricAppId
                },
                httpsAgent,
                timeout: 30000
            });
            ...
            if (response.data && response.data.token) {
                logger_1.default.info('Telebirr token obtained successfully');
    ...
    static async initialize(params) {
        try {
            if (this.isTestMode()) {
                ...
                logger_1.default.info({ prepayId }, 'Telebirr test mode initialization');
            ...
            const bizContent = {
                appid: env_1.env.teleBirrMerchantAppId || '',
                merch_code: env_1.env.teleBirrShortCode || '',
            ...
            const requestData = {
                ...
                nonce_str: crypto_1.default.randomBytes(32).toString('hex'),
            ...
            const gatewayUrl = this.getGatewayUrl();
            const response = await axios_1.default.post(`${gatewayUrl}/payment/v1/merchant/preOrder`, finalRequestData, {
                headers: {
                    'Content-Type': 'application/json',
                    'X-APP-Key': env_1.env.teleBirrFabricAppId,
                    'Authorization': token
            ...
            if (response.data && response.data.code === '0') {
                ...
                if (typeof bizResponse === 'string') {
                    try {
                        ...
                        logger_1.default.warn({ bizResponse }, 'Failed to parse biz_content as JSON');
                    }
                }
                ...
                if (prepayId) {
                    ...
                    const checkoutParams = new URLSearchParams({
                        ...
                        merch_code: env_1.env.teleBirrShortCode || '',
                        prepay_id: prepayId,
                        nonce_str: crypto_1.default.randomBytes(16).toString('hex'),
                        timestamp: Math.floor(Date.now() / 1000).toString(),
                        sign_type: 'SHA256WithRSA',
                        version: '1.0'
                    });
     ...
     */
    static async verify(outTradeNo) {
        try {
            if (this.isTestMode()) {
            ...
            logger_1.default.info({ outTradeNo }, 'Verifying Telebirr payment');
            if (!this.isConfigured()) {
                return {
                    success: false,
            ...
            const gatewayUrl = this.getGatewayUrl();
            const response = await axios_1.default.post(`${gatewayUrl}/payment/v1/merchant/queryOrder`, finalRequestData, {
                headers: {
                    'Content-Type': 'application/json',
                    'X-APP-Key': env_1.env.teleBirrFabricAppId,
                    'Authorization': token
            ...
            if (response.data && response.data.code === '0') {
                ...
                const status = bizResponse?.trade_status ||
                    ...
                    bizResponse?.status ||
                    bizResponse?.result;
                logger_1.default.debug({ status, bizResponse }, 'Payment status check');
    ...
    static isConfigured() {
        try {
            if (this.isTestMode()) {
                logger_1.default.info('Telebirr running in test mode');
                return true;
            }
            const requiredVars = [
            ...
            for (const varName of requiredVars) {
                const value = env_1.env[varName];
                if (!value) {
                    logger_1.default.warn({ missingVar: varName }, 'Telebirr configuration missing');
                    return false;
                }
            ...
            if (env_1.env.teleBirrPrivateKey && env_1.env.teleBirrPrivateKey.length < 100) {
                logger_1.default.warn({
                    keyLength: env_1.env.teleBirrPrivateKey.length
                }, 'Telebirr private key seems too short');
                return false;
            }
            logger_1.default.debug('Telebirr configuration check passed');
            return true;
        }
        catch (error) {
            logger_1.default.error({ error }, 'Telebirr configuration check error');
            return false;
        }
    }
    /**
     ...
     */
    static isTestMode() {
        const value = (env_1.env.teleBirrTestMode || '').toLowerCase();
    ...
    static async testConnection() {
        try {
            logger_1.default.info('Testing Telebirr connection');
            if (this.isTestMode()) {
                return {
                    success: true,
            ...
            return {
                ...
                details: {
                    ...
                    appId: env_1.env.teleBirrMerchantAppId ? '***' + env_1.env.teleBirrMerchantAppId.slice(-4) : 'missing',
                    shortCode: env_1.env.teleBirrShortCode,
                    gatewayUrl: this.getGatewayUrl()
                }
            };
        }
     ...
     */
    static validateWebhook(signature, payload) {
        try {
            logger_1.default.info({
            ...
            logger_1.default.error({ error }, 'Webhook validation error');
            return false;
        }
    }
}
...
TelebirrProvider.H5_CHECKOUT_BASE = 'https://196.188.120.3:38443';
