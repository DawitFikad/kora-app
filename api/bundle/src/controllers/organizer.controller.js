...
const event_operations_service_1 = require("../services/event-operations.service");
...
class OrganizerController {
    ...
    static async getDashboard(req, res) {
        try {
            ...
            const dashboard = await event_operations_service_1.EventOperationsService.getEventOperationsSummary(eventId, organizerId);
    ...
    static async getOverview(req, res) {
        try {
            ...
            const summary = await event_operations_service_1.EventOperationsService.getOrganizerSummary(organizerId);
            ...
            logs.forEach(log => {
                ...
                if (!eventStatusMap.has(eid)) {
                    if (meta.type === 'FEATURE_REQUEST')
                        eventStatusMap.set(eid, 'PENDING');
                    else if (meta.type === 'FEATURE_DECLINED')
                        eventStatusMap.set(eid, 'REJECTED');
                }
            });
    ...
    static async resendTicket(req, res) {
        try {
            ...
            let code = ticket.id;
            try {
                const payload = jwt.default.verify(ticket.qrPayload, secret);
                if (payload?.code)
                    code = payload.code;
            }
            catch {
            ...
            const { NotificationChannel } = await Promise.resolve().then(() => __importStar(require("@prisma/client")));
            ...
            try {
                ...
                if (msg.includes('unknown arg') || (msg.includes('column') && msg.includes('does not exist'))) {
                    try {
                        ...
                        await prisma.$executeRawUnsafe('UPDATE "Ticket" SET "isVip" = $1 WHERE id = $2', !!isVip, ticketId);
    ...
    static async getFinancials(req, res) {
        try {
            ...
            if (!organizerId)
                return res.status(403).json({ success: false, message: "Unauthorized" });
            const financials = await event_operations_service_1.EventOperationsService.getOrganizerFinancials(organizerId);
    ...
    static async getTicketStats(req, res) {
        try {
            ...
            if (!organizerId)
                return res.status(403).json({ success: false, message: "Unauthorized" });
            const stats = await event_operations_service_1.EventOperationsService.getOrganizerTicketStats(organizerId);
     ...
     */
    static async updateTicketTier(req, res) {
        try {
            const organizerId = req.user.organizerId;
            if (!organizerId)
                return res.status(403).json({ success: false, message: "Unauthorized" });
    ...
    static async updateSettings(req, res) {
        try {
            ...
            if (!organizerId)
                return res.status(403).json({ success: false, message: "Unauthorized" });
            const { organizationName, contactEmail, contactPhone, city, payoutDetails, adminNote, description, websiteUrl, socialLinks, supportPhone, supportEmail, businessAddress, categoryFocus, operatingCities, defaultConfig, notificationPrefs } = req.body;
            const prisma = (await Promise.resolve().then(() => __importStar(require("../lib/prisma")))).prisma;
            ...
            if (currentProfile?.status !== 'APPROVED') {
                ...
                const attemptedRestrictedUpdate = (organizationName && organizationName !== currentProfile.organizationName) ||
     ...
     */
    static async requestTwoFactorOtp(req, res) {
        try {
            const organizerId = req.user.organizerId;
            if (!organizerId)
                return res.status(403).json({ success: false, message: "Unauthorized" });
            ...
            const otp = await otp_service_1.OtpService.generateOtp(phoneNumber);
            await sms_service_1.SmsService.sendOtp(phoneNumber, otp);
            await redis_1.default.setex(`2fa_action:${organizerId}`, 300, action);
     ...
     */
    static async verifyTwoFactorOtp(req, res) {
        try {
            const organizerId = req.user.organizerId;
            if (!organizerId)
                return res.status(403).json({ success: false, message: "Unauthorized" });
            ...
            const expected = await redis_1.default.get(`2fa_action:${organizerId}`);
            ...
            const isValid = await otp_service_1.OtpService.verifyOtp(phoneNumber, otp);
            if (!isValid) {
                return res.status(400).json({ success: false, message: "Invalid or expired OTP" });
            }
            const updatedConfig = { ...(profile?.defaultConfig || {}), twoFactorEnabled: action === 'enable' };
            await prisma.organizerProfile.update({
                where: { id: organizerId },
                data: { defaultConfig: updatedConfig }
            });
            await redis_1.default.del(`2fa_action:${organizerId}`);
    ...
    static async createEvent(req, res) {
        try {
            ...
            const baseData = {
                ...
                additionalDates: Array.isArray(additionalDates) ? additionalDates.map((d) => new Date(d)) : [],
            ...
            let event;
            try {
                event = await prisma.event.create({ data: baseData, include: { tiers: true } });
            }
            catch (error) {
                const msg = (error?.message || '').toLowerCase();
                if (msg.includes('additionaldates') || msg.includes('ispublic') || msg.includes('unknown arg') || (msg.includes('column') && msg.includes('does not exist'))) {
                    ...
                    if (fallbackData.tiers?.create) {
                        fallbackData.tiers = {
                            create: fallbackData.tiers.create.map((tier) => ({
                                name: tier.name,
                                price: tier.price,
                    ...
                    event = await prisma.event.create({ data: fallbackData, include: { tiers: true } });
                }
                else {
                    throw error;
                }
    ...
    static async createPromoCode(req, res) {
        try {
            ...
            const promo = await promo_code_service_1.PromoCodeService.create({
                code,
        ...
        try {
            ...
            if (promos.length > 0) {
                ...
                purchases.forEach(purchase => {
                    ...
                    if (!promoId || !promoStats.has(promoId))
                        return;
    ...
    static async deletePromoCode(req, res) {
        try {
            ...
            await promo_code_service_1.PromoCodeService.delete(promoId);
    ...
    static async updateEvent(req, res) {
        try {
            ...
            if (titleAm !== undefined)
                updateData.titleAm = titleAm;
            if (description !== undefined)
                updateData.description = description;
            if (descriptionAm !== undefined)
                updateData.descriptionAm = descriptionAm;
            if (venue)
                updateData.venue = venue;
            if (dateTime)
                updateData.dateTime = new Date(dateTime);
            if (Array.isArray(additionalDates))
                updateData.additionalDates = additionalDates.map((d) => new Date(d));
            ...
            if (refundPolicy !== undefined)
                updateData.refundPolicy = refundPolicy;
            if (minAge !== undefined)
                updateData.minAge = parseInt(minAge);
            if (additionalPolicy !== undefined)
                updateData.additionalPolicy = additionalPolicy;
            if (hasSeatMap !== undefined)
                updateData.hasSeatMap = hasSeatMap;
            if (status)
            ...
            if (tiers && Array.isArray(tiers)) {
                ...
                const updates = incoming.filter(t => t.id).map(t => (prisma.ticketTier.update({
                ...
                // Create new tiers
                const creates = incoming.filter(t => !t.id).map(t => (prisma.ticketTier.create({
                    data: {
                        eventId,
                        name: t.name,
                        type: t.type || 'GENERAL',
                ...
                try {
                    ...
                    const msg = (error?.message || '').toLowerCase();
                    if (msg.includes('unknown arg') || (msg.includes('column') && msg.includes('does not exist'))) {
                        const fallbackUpdates = incoming.filter(t => t.id).map(t => (prisma.ticketTier.update({
                            where: { id: Number(t.id) },
                            data: {
                                name: t.name,
                                price: parseFloat(t.price),
                        ...
                        const fallbackCreates = incoming.filter(t => !t.id).map(t => (prisma.ticketTier.create({
                            data: {
                                eventId,
                                name: t.name,
                                price: parseFloat(t.price),
    ...
    static async duplicateEvent(req, res) {
        try {
            ...
            const duplicateData = {
                ...
                additionalDates: original.additionalDates || [],
            ...
            let copy;
            try {
                copy = await prisma.event.create({ data: duplicateData });
            }
            catch (error) {
                const msg = (error?.message || '').toLowerCase();
                if (msg.includes('additionaldates') || msg.includes('ispublic') || msg.includes('unknown arg') || (msg.includes('column') && msg.includes('does not exist'))) {
                    ...
                    if (fallback.tiers?.create) {
                        fallback.tiers = {
                            create: fallback.tiers.create.map((tier) => ({
                                name: tier.name,
                                price: tier.price,
                                capacity: tier.capacity,
                                salesStart: tier.salesStart,
                    ...
                    copy = await prisma.event.create({ data: fallback });
                }
                else {
                    throw error;
                }
     ...
     */
    static async requestFeature(req, res) {
        try {
    ...
    static async contactSupport(req, res) {
        try {
            ...
            await EmailService.sendEmail(supportEmail, `[Support] ${organizer.organizationName}: ${subject}`, `New support request from ${organizer.organizationName} (ID: ${organizer.id}).\n\nContact: ${organizer.contactEmail || 'N/A'}\nPhone: ${organizer.contactPhone || 'N/A'}\n\nMessage:\n${message}`, supportHtml);
            ...
            if (organizer.contactEmail) {
                ...
                await EmailService.sendEmail(organizer.contactEmail, `Support Request Confirmation: ${subject}`, `Dear ${organizer.organizationName},\n\nThank you for contacting ET-Ticket support. We have received your request and will respond within 24 hours.\n\nYour Message:\n${message}\n\nBest regards,\nET-Ticket Support Team`, confirmationHtml);
            }
    ...
    static async requestPhoneChange(req, res) {
        try {
            ...
            const prisma = (await Promise.resolve().then(() => __importStar(require("../lib/prisma")))).prisma;
            const OtpService = (await Promise.resolve().then(() => __importStar(require("../services/otp.service")))).OtpService;
            const SmsService = (await Promise.resolve().then(() => __importStar(require("../services/sms.service")))).SmsService;
     ...
     */
    static async getRefundImpact(req, res) {
        try {
            const organizerId = req.user?.organizerId;
            if (!organizerId)
                return res.status(403).json({ error: "Unauthorized" });
