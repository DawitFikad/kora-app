...
class PaymentController {
    ...
    static async initialize(req, res) {
        try {
            ...
            const result = await payment_service_1.PaymentService.initializePayment(parseInt(purchaseId));
            res.json(result);
        }
        catch (error) {
    ...
    static async healthCheck(req, res) {
        try {
            const config = {
                chapaConfigured: !!(env_1.env.chapaSecretKey && !env_1.env.chapaSecretKey.includes('your_')),
                telebirrConfigured: !!(env_1.env.teleBirrMerchantAppId && !env_1.env.teleBirrMerchantAppId.includes('your_')),
    ...
    static async verify(req, res) {
        try {
            ...
            const result = await payment_service_1.PaymentService.verifyPayment(paymentRef, externalRef);
            res.json({
    ...
    static async webhook(req, res) {
        try {
            ...
            if (chapaSignature) {
                ...
                const isValid = payment_service_1.PaymentService.validateChapaWebhook(chapaSignature, payload.toString());
                if (!isValid)
            ...
            if (status === "SUCCESS" && paymentRef) {
                await payment_service_1.PaymentService.verifyPayment(paymentRef, externalRef);
            }
    ...
    static async verifyCallback(req, res) {
        try {
            const { ref } = req.query;
            const defaultUrl = `${env_1.env.apiUrl}/api/payments/completion`;
            const clientUrl = process.env.CLIENT_URL || defaultUrl;
            if (!ref) {
                res.redirect(`${clientUrl}?status=error&message=Missing+Payment+Reference`);
                return;
            }
            const result = await payment_service_1.PaymentService.verifyPayment(ref);
    ...
    static async completion(req, res) {
        ...
        const deepLink = isSuccess
            ...
            : `etticket://payment/failed?reason=${encodeURIComponent(message || 'Payment failed')}`;
        res.send(`
            <html>
                <head>
