const crypto = require('crypto');
const axios = require('axios');
const https = require('https');

const env = {
    teleBirrMerchantAppId: "1547579123609609",
    teleBirrFabricAppId: "c4182ef8-9249-458a-985e-06d191f4d505",
    teleBirrShortCode: "402666",
    teleBirrAppSecret: "fad0f06383c6297f545876694b974599",
    teleBirrPrivateKey: ""
};

function formatPrivateKey(key) {
    let cleanKey = key.replace(/-----BEGIN (RSA )?PRIVATE KEY-----/g, '').replace(/-----END (RSA )?PRIVATE KEY-----/g, '').replace(/\s+/g, '');
    const chunked = cleanKey.match(/.{1,64}/g)?.join('\n');
    return `-----BEGIN PRIVATE KEY-----\n${chunked}\n-----END PRIVATE KEY-----`;
}

async function run() {
    const bizContent = {
        out_trade_no: "T" + Date.now(),
        subject: "Test",
        total_amount: "1.00",
        short_code: env.teleBirrShortCode,
        notify_url: "https://google.com/", // A valid public URL
        return_url: "https://google.com/",
        timeout_express: "30"
    };

    const requestData = {
        appid: env.teleBirrMerchantAppId,
        biz_content: JSON.stringify(bizContent),
        nonce: crypto.randomBytes(16).toString('hex'),
        timestamp: Math.floor(Date.now() / 1000).toString(),
        method: "payment.preorder"
    };

    const sortedKeys = Object.keys(requestData).sort();
    const signString = sortedKeys.map(k => `${k}=${requestData[k]}`).join('&');
    const signer = crypto.createSign('RSA-SHA256');
    signer.update(signString);
    requestData.sign = signer.sign(formatPrivateKey(env.teleBirrPrivateKey), 'base64');
    requestData.sign_type = "SHA256WithRSA";

    const agent = new https.Agent({ rejectUnauthorized: false });

    try {
        const res = await axios.post('https://196.188.120.3:38443/payment/v1/merchant/preOrder', requestData, {
            headers: {
                'Content-Type': 'application/json',
                'Host': 'developerportal.ethiotelebirr.et'
            },
            httpsAgent: agent
        });
        console.log("RESULT:", res.data);
    } catch (e) {
        console.log("ERR:", e.response ? e.response.status : e.message, e.response ? e.response.data : '');
    }
}

run();
