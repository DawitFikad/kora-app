...
async function testLegacy() {
    ...
    for (const path of paths) {
        try {
            console.log(`\nTesting Legacy on ${path}...`);
            const res = await axios.post(`https://196.188.120.3:38443${path}`, payload, {
                headers: {
            ...
            console.log(`FAIL:`, err.response ? err.response.status : err.message, err.response ? err.response.data : '');
        }
    }
}
