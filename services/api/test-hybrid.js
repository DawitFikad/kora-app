const crypto = require('crypto');
const axios = require('axios');
const https = require('https');

const env = {
    teleBirrMerchantAppId: "1547579123609609",
    teleBirrFabricAppId: "c4182ef8-9249-458a-985e-06d191f4d505",
    teleBirrShortCode: "402666",
    teleBirrAppSecret: "fad0f06383c6297f545876694b974599",
    teleBirrPrivateKey: ""
};

function formatPrivateKey(key) {
    let cleanKey = key.replace(/-----BEGIN (RSA )?PRIVATE KEY-----/g, '').replace(/-----END (RSA )?PRIVATE KEY-----/g, '').replace(/\s+/g, '');
    const chunked = cleanKey.match(/.{1,64}/g)?.join('\n');
    return `-----BEGIN PRIVATE KEY-----\n${chunked}\n-----END PRIVATE KEY-----`;
}

function encrypt(data, key) {
    const cipher = crypto.createCipheriv('aes-128-ecb', key.substring(0, 16), '');
    return Buffer.concat([cipher.update(JSON.stringify(data)), cipher.final()]).toString('base64');
}

function sign(data, key) {
    const sortedKeys = Object.keys(data).sort();
    const signString = sortedKeys.map(k => `${k}=${data[k]}`).join('&');
    const signer = crypto.createSign('RSA-SHA256');
    signer.update(signString);
    return signer.sign(formatPrivateKey(key), 'base64');
}

async function run() {
    const requestData = {
        appid: env.teleBirrFabricAppId,
        merAppid: env.teleBirrMerchantAppId,
        nonce: crypto.randomBytes(16).toString('hex'),
        notifyUrl: "http://example.com/notify",
        outTradeNo: "T" + Date.now(),
        returnUrl: "http://example.com/return",
        shortCode: env.teleBirrShortCode,
        subject: "Test",
        timeoutExpress: '30',
        timestamp: Date.now().toString(),
        totalAmount: "1.00",
        tradeType: 'InApp'
    };

    const payload = {
        appid: env.teleBirrFabricAppId,
        sign: sign(requestData, env.teleBirrPrivateKey),
        ussd: encrypt(requestData, env.teleBirrAppSecret)
    };

    const agent = new https.Agent({ rejectUnauthorized: false });

    try {
        const url = 'https://196.188.120.3:38443/apiaccess/payment/gateway/payment/v1/merchant/preOrder';
        console.log("Testing:", url);
        const res = await axios.post(url, payload, {
            headers: {
                'Content-Type': 'application/json',
                'Host': 'developerportal.ethiotelebirr.et'
            },
            httpsAgent: agent,
            timeout: 5000
        });
        console.log("RESULT:", res.data);
    } catch (e) {
        console.log("ERR:", e.response ? e.response.status : e.message, e.response ? e.response.data : '');
    }
}

run();
