const crypto = require('crypto');
const axios = require('axios');
const https = require('https');

const env = {
    numeric: "1547579123609609",
    uuid: "c4182ef8-9249-458a-985e-06d191f4d505",
    secret: "fad0f06383c6297f545876694b974599",
    privateKey: ""
};

function formatKey(k) {
    const chunked = k.match(/.{1,64}/g).join('\n');
    return `-----BEGIN PRIVATE KEY-----\n${chunked}\n-----END PRIVATE KEY-----`;
}

async function run() {
    const biz = {
        out_trade_no: "T" + Date.now(),
        subject: "Test",
        total_amount: "1.00",
        short_code: "402666"
    };

    const req = {
        appid: env.numeric,
        biz_content: JSON.stringify(biz),
        nonce: "123456",
        timestamp: Math.floor(Date.now() / 1000).toString(),
        method: "payment.preorder"
    };

    const s = Object.keys(req).sort().map(k => `${k}=${req[k]}`).join('&');
    const signer = crypto.createSign('RSA-SHA256');
    signer.update(s);
    req.sign = signer.sign(formatKey(env.privateKey), 'base64');
    req.sign_type = "SHA256WithRSA";

    const endpoints = [
        'https://196.188.120.3:38443/payment/v1/merchant/preOrder',
        'http://196.188.120.3:11443/payment/v1/merchant/preOrder'
    ];

    for (const url of endpoints) {
        try {
            console.log(`Testing ${url}...`);
            const res = await axios.post(url, req, {
                headers: { 'Host': 'developerportal.ethiotelebirr.et' },
                httpsAgent: new https.Agent({ rejectUnauthorized: false }),
                timeout: 5000
            });
            console.log("SUCCESS:", JSON.stringify(res.data));
            process.exit(0);
        } catch (e) {
            console.log("FAIL:", e.response ? e.response.status : e.message, e.response ? e.response.data : '');
        }
    }
}

run();
