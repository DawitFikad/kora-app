const fs = require('fs');
const { spawn } = require('child_process');
const path = require('path');

// Raw values - no parsing to avoid issues
const envs = {
    DATABASE_URL: "postgresql://postgres.ywgoqqbqemcevgdyozca:*4a%2FhGRdL9QV7Nn@aws-1-eu-west-1.pooler.supabase.com:5432/postgres?pgbouncer=true",
    DIRECT_URL: "postgresql://postgres.ywgoqqbqemcevgdyozca:*4a%2FhGRdL9QV7Nn@aws-1-eu-west-1.pooler.supabase.com:5432/postgres",
    TELEBIRR_API_URL: "https://196.188.120.3:38443/apiaccess/payment/gateway",
    TELEBIRR_APP_SECRET: "fad0f06383c6297f545876694b974599",
    TELEBIRR_FABRIC_APP_ID: "c4182ef8-9249-458a-985e-06d191f4d505",
    TELEBIRR_MERCHANT_APP_ID: "1547579123609609",
    TELEBIRR_PRIVATE_KEY: "",
    TELEBIRR_SHORT_CODE: "402666",
    JWT_SECRET: "super-secret-key-change-this-in-prod",
    CHAPA_SECRET_KEY: "CHASECK_TEST-GwIVdzkZKmegLw90iu7L75dCScli8aWY",
    API_URL: "https://et-ticket-api-v2.vercel.app",
    CLIENT_URL: "https://et-ticket-api-v2.vercel.app/api/payments/completion"
};

async function setEnv(key, value) {
    return new Promise((resolve, reject) => {
        console.log(`Setting ${key}...`);

        // First remove (ignore error)
        const rm = spawn('npx', ['vercel', 'env', 'rm', key, 'production', '-y'], { stdio: 'inherit', shell: true });

        rm.on('close', () => {
            // Then add
            const add = spawn('npx', ['vercel', 'env', 'add', key, 'production'], { stdio: ['pipe', 'inherit', 'inherit'], shell: true });

            add.stdin.write(value); // Write RAW value, no newline
            add.stdin.end();

            add.on('close', (code) => {
                if (code === 0) {
                    console.log(`✅ ${key} set successfully`);
                    resolve();
                } else {
                    reject(new Error(`Failed to set ${key}`));
                }
            });
        });
    });
}

(async () => {
    for (const [key, value] of Object.entries(envs)) {
        try {
            await setEnv(key, value);
        } catch (e) {
            console.error(e);
        }
    }
})();
